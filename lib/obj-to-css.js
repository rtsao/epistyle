...
function processNested(name, nested, query, important) {
  ...
  Object.keys(nested).forEach(function(key) {
    var val = nested[key];
    if (isMediaQuery(key)) {
      media.push(processObj(name, val, key, important));
    } else {
      nonMedia.push(processObj({
        selector: name.selector + key,
        prefix: name.prefix
      }, val, query, important));
    }
  });
  return nonMedia.concat(media);
}
...
function stringify(name, decls, query, important) {
  return query ? makeMediaQuery(name, decls, query, important) : makeClass(name, decls, important);
}
...
function declaration(property, value, important) {
  var decl = dashify(property).concat(': ').concat(value);
